"""Abstract storage backend protocol for graph-mem.
...
class StorageBackend(ABC):
    ...
    async def fetch_relationships_between(self, entity_ids: list[str]) -> list[dict[str, Any]]:
        ...
        Returned rows include ``source_name``, ``source_type``,
        ``target_name``, and ``target_type`` resolved from the entities table.
        """
